<template>
    <li
            ...
            class="tags-list-li"
            @click="">
        <!--<span class="text-liste-autocomplete">{{ tag.nom }}</span>-->
        <router-link class="text-liste-autocomplete" :to="{ name: 'tag-category', params:{id: tag.id, nameTag: tag.nom}}">{{ tag.nom }}</router-link>
        <span v-if="tag.repertorie" class="tag-icon">
